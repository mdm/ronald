//! Unoptimized reference implementation of Screen
...
const BUFFER_LENGTH: usize = SCREEN_BUFFER_WIDTH * SCREEN_BUFFER_HEIGHT * 4;
...
impl ReferenceScreen {
    ...
    pub fn write(&mut self, color: usize) {
        ...
        if y >= 4 * 16 && x < 48 * 16 {
            let y = y - (4 * 16);
            let start = (y * SCREEN_BUFFER_WIDTH + x) * 4 % BUFFER_LENGTH;
            self.buffer[start..(start + 4)].copy_from_slice(&color);
            let start = (y * SCREEN_BUFFER_WIDTH + x + SCREEN_BUFFER_WIDTH) * 4 % BUFFER_LENGTH;
            self.buffer[start..(start + 4)].copy_from_slice(&color);
        }
