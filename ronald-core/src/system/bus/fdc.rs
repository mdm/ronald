    ...
    fn execute_command(&mut self) {
        ...
        match command {
            Command::ReadData {
                ...
                match &self
                    .drives
                    .get(unit_select as usize)
                    .and_then(|drive| drive.disk)
                {
                    Some(disk) => {
                        let track = match disk.find_track_index(cylinder_number, head_address) {
                            Some(track) => track,
                            None => unimplemented!(), // TODO: handle error, send NO DATA and set ST0 to "01"/"cancelled due to error"
                        };
